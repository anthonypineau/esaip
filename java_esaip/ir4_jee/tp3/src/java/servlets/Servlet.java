...
public class Servlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            process(data, incommingData);
            request.setAttribute( "data", data );
            request.getRequestDispatcher("/WEB-INF/result.jsp").forward(request, response);
        }
    }
    
    private void process(ArrayList<Data> data, Map<String, String[]> incommingData) throws IntrospectionException, IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        BeanInfo infoData = Introspector.getBeanInfo(Data.class);
       // String[] temp =(String[])(incommingData.values().toArray()[0]);
        for(int i=0;i<((String[])(incommingData.values().toArray()[0])).length;i++){
            data.add(new Data());
        }
        ...
        for(Data bean : data){
            bean.setBonus(bonusCalculation(bean.getMultiplier()));
        }
    }
